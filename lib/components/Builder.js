...
export default class Builder extends PureComponent {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const style = getCSSCode(cur, containerId);
    const minifiedStyle = style.replace(/(\r\n|\n|\r)/gm, "");
    const obj = isDashboardBuilderActive ? { theme: cur, style: minifiedStyle, options: getJSCode(cur, 'object') } : cur;
  ...
  renderChoroplethMapDropdown() {
    const defaultMap = choroplethMapOptions.find(option => option.value === this.state['choropleth_map']);
    return (
      <div className="keen-theme-builder__row">
  ...
  renderMapBorderSettings() {
    const defaultBorderSize = lineThicknessOptions.find(option => option.value === this.state['choropleth_border_size']);
    return (
      <>
        <div className="accordion__item__row">
          ...
          <Select
            ...
            options={lineThicknessOptions}
            onChange={e => this.handleTextStateUpdate('choropleth_border_size', e.value)}
            className="predefined-palette"
            classNamePrefix="custom-dropdown"
          />
        </div>
        <div className="accordion__item__row">
          <ColorPicker title="Chart border" initialColor={this.state['choropleth_border_color']} defaultColor={defaultColors.border} onChange={e => this.handleStateChange('choropleth_border_color', e.hex)}/>
        </div>
      </>
    )
  }
  ...
  renderChoroplethSettings() {
    return (
      <>
        <Switcher checked={!!this.state['choropleth_border_show']} onChange={() => this.handleStateChange('choropleth_border_show', !this.state['choropleth_border_show'])} text={{ on: 'Show', off: 'Hide' }}>
  ...
  renderFunnelPercentsSection() {
    const defaultCountingMethod = funnelCountingMethodOptions.find(option => option.value === this.state['funnel_percents_count']);
    return (
      <>
        <div className="accordion__item__row">
          <span className="control-label">Counting method</span>
          <Select
            defaultValue={defaultCountingMethod || funnelCountingMethodOptions[0]}
            options={funnelCountingMethodOptions}
            onChange={e => this.handleTextStateUpdate('funnel_percents_count', e.value)}
            className="predefined-palette"
            classNamePrefix="custom-dropdown"
          />
        </div>
        <div className="accordion__item__row">
          <FormInput key="funnel-percents-decimals" type="text" label="Decimals" name="funnel-percents-decimals" value={this.state['funnel_percents_decimals']} onChange={e => this.handleTextStateUpdate('funnel_percents_decimals', e.target.value)} maxLength="2" />
        </div>
      </>
    )
  }
  ...
  renderFunnel3dOptions() {
    const default3dOption = funnelEffect3dOptions.find(option => option.value === this.state['funnel_effect3d']);
    return (
      <div className="accordion__item__row">
  ...
  renderConfigSection() {
    ...
    return (
      <section className="builder__config">
        ...
        { this.state.chart === 'choropleth' && this.renderChoroplethMapDropdown() }
        {(this.state.chart === 'heatmap' || this.state.chart === 'choropleth') &&
          <Switcher checked={!!this.state['chart_slider_show']} onChange={() => this.handleStateChange('chart_slider_show', !this.state['chart_slider_show'])} text={{ on: 'Show', off: 'Hide' }}>
        ...
        <Accordion>
          {sections.appearance &&
            <div label="Appearance">
              ...
              {this.state.chart === 'choropleth' && this.renderChoroplethSettings()}
              <div className="accordion__item__row">
          ...
          {sections.funnel &&
            <div label="Funnel">
              ...
              {this.renderFontStyle('Label format', 'funnel_label', true)}
              <Switcher checked={!!this.state['funnel_lines']} onChange={() => this.handleStateChange('funnel_lines', !this.state['funnel_lines'])} text={{ on: 'Show', off: 'Hide' }}>
                <span className="control-label">Lines</span>
              </Switcher>
              <Switcher checked={!!this.state['funnel_results']} onChange={() => this.handleStateChange('funnel_results', !this.state['funnel_results'])} text={{ on: 'Show', off: 'Hide' }}>
                <span className="control-label">Results</span>
              </Switcher>
              <Switcher checked={!!this.state['funnel_percents_show']} onChange={() => this.handleStateChange('funnel_percents_show', !this.state['funnel_percents_show'])} text={{ on: 'Show', off: 'Hide' }}>
                <span className="section-title">Percents</span>
              </Switcher>
              {this.state['funnel_percents_show'] && this.renderFunnelPercentsSection()}
              <Switcher checked={!!this.state['funnel_hover']} onChange={() => this.handleStateChange('funnel_hover', !this.state['funnel_hover'])} text={{ on: 'Show', off: 'Hide' }}>
                <span className="control-label">Hover</span>
              </Switcher>
              <Switcher checked={!!this.state['funnel_margin']} onChange={() => this.handleStateChange('funnel_margin', !this.state['funnel_margin'])} text={{ on: 'Show', off: 'Hide' }}>
                <span className="control-label">Margin between steps</span>
              </Switcher>
              <div className="accordion__item__row">
                <FormInput key="funnel-minimal-size" type="number" label="Minimal size" name="funnel-minimal-size" value={this.state['funnel_minimal_size']} onChange={e => this.handleTextStateUpdate('funnel_minimal_size', e.target.value)} />
              </div>
              {(this.state['chart'] === 'funnel-3d' || this.state['chart'] === 'horizontal-funnel-3d') && this.renderFunnel3dOptions()}
            </div>
          }
        </Accordion>
      </section>
  ...
  renderTabChart() {
    ...
    const choropleth = this.state['chart'] === 'choropleth' ?
    {
    ...
    const funnel = funnels.includes(this.state['chart']) ?
    {
