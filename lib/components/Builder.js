...
export default class Builder extends PureComponent {
  ...
  handleChartTypeChange = (data) => {
    ...
    const sections = getAccordionSections(data.value);
    this.setState({
      chart: data.value,
  ...
  handleChartPaletteChange = (data) => {
    ...
    this.setState({ chartPalette: data.value, colors });
  }
        ...
        <Accordion>
          ...
          {sections.tooltip &&
            <div label="Tooltip">
              ...
              <div className="accordion__item__row">
                <ColorPicker key="tooltip.background" title="Tooltip background" initialColor={this.state['tooltip.background']} onChange={e => this.handleTextStateUpdate('tooltip.background', e.hex)} />
              </div>
              <div className="accordion__item__row">
