...
const TabChart = ({type, colors = [], legend = {}, axis = {}, title, subtitle, data, point, prefix, suffix, choropleth, funnel, slider, simpleTooltip, sparkline, stacked, pagination, ...props}) => {
  const chartSpecificLabels = (type) => {
    const getLabel = () => (data && data.labels) ? { label: { show: true } } : { label: { show: false } };
  ...
  const renderChart = () => {
    ...
    return (
      <Chart
        ...
        stacked={stacked}
        table={tableOptions}
        results={(type === 'choropleth' && choropleth.map === 'us') ? results['choropleth-us'] : results[type]}
