...
export function getCSSRules(state) {
  return {
    ...
    tooltipBackground : state['tooltip.background'] ? `background-color: ${state['tooltip.background']};` : 'background-color: inherit;',
    tooltipBorder : state['tooltip.border'] ? `border-color: ${state['tooltip.border']};` : 'border-color: inherit;',
  }
}
