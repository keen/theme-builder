...
export function getCSSCode(data) {
...
export function getJSCode(data, type = 'string') {
  ...
  const subtitle = data['subtitle_text'] ? data['subtitle_text'] : false;
  ...
  const pointOptions = data['series_points_show'] ? { show: data['series_points_show'], r: data['series_points_size'] } : { show: data['series_points_size'] };
  ...
  return type === 'object' ? obj : `const chart = new KeenDataviz(${JSON.stringify(options, null, 2)})`;
}
